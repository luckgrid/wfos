#!/usr/bin/env bash
# validate-dotfiles.sh — dotfiles validation entry point (moon: panoply:validate-dotfiles).
#
...
if ! command -v chezmoi >/dev/null 2>&1; then
  ...
  printf 'Install: brew install chezmoi  (or panoply bootstrap / dotfiles module)\n'
  exit 0
fi
...
if [ "$APPLY_ISOLATED" = true ]; then
  ...
  HOME="$test_home" WFOS_PROFILE=agent-safe PANOPLY_HOME="$PANOPLY_HOME" \
